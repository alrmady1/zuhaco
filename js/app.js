...
function renderApp() {
  ...
  // المظهر المحفوظ يعيش في DB_CACHE (الخادم) — يجب تطبيقه هنا بعد اكتمال التحميل، وليس فقط عند DOMContentLoaded حيث تكون البيانات لم تُحمَّل بعد
  applyTheme();

  // تُنفَّذ مرة واحدة فقط لكل تشغيل حقيقي للتطبيق (بعد اكتمال تحميل DB_CACHE من الخادم)
  // ملاحظة: seedIfEmpty() أُزيلت من هنا عمداً — في نموذج الخادم المركزي، تشغيلها من أي
  // جهاز/جلسة قد يستبدل بيانات حقيقية مشتركة ببيانات تجريبية (حادثة حصلت فعلاً). أي تهيئة
  // بيانات تجريبية أولية تتم فقط يدوياً وبوعي تام إن لزم الأمر مستقبلاً، وليس تلقائياً هنا.
  if (!APP_BOOTSTRAPPED) {
    migrateClients();
    migratePriceCatalog();
    migrateProfitMargin();
